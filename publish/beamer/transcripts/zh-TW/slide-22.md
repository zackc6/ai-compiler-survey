...
**C — 型別化工具介面。**
必要的行動空間。**MCP**（Model Context Protocol——工具伺服器標準）級伺服器、`mlir-opt-repl`、CompileIQ skills、FlashInfer Trace `apply()`——智慧體在編譯器會驗證的 schema 內提案。**面向 LLM 的 IR** 家族：摘要／指紋、意圖／行動、型別化智慧體 IR（Cake／Argus）——不是貼上 `.ll`。T1（型別化智慧體↔編譯器介面）解鎖 C3（自由改寫 vs 建議）、C5（預設路徑）、C6（替換 vs 控制面）。Lean：窄 ACF 與 hint，不是自由 IR 改寫。
