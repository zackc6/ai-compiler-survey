...
**Google / DeepMind。**
AlphaEvolve Cloud——雲上的演化編碼。Magellan + MLGO 並行（C1）。**MaxKernel**——Pallas/TPU 峰值智慧體，編譯器與 XProf 接納；不是 ASIC 覆蓋（C9 仍開）。訊號：離線啟發式、樹內顧問，與 TPU 核心智慧體共存。
...
**AMD。**
**GEAK v4**——在熱的 sglang/vLLM 上做 Amdahl 分流。**Hyperloom**（2026 年 9 月 21 日）坐在上面：先做框架層，再選一個核心後端（GEAK 或 KernelForge），Critic 決定保留或回退，任務寫在狀態檔。作者在 16 個負載上的中位數 1.73×。訊號：追 serving **F**，不只核心 microbench。仍不是公開 p50 預設路徑（**C2**）。
