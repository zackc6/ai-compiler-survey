...
**3. 智慧體可見的 IR：傾印 vs 摘要／Cake／Argus／TIRx／Tile？**
T1 型別化介面你先投哪裡——以及 LLM 會看到*哪張臉*？傾印 LLVM/MLIR（LLM4IR：CFG／執行失敗）vs 摘要、IntOpt 式意圖、Cake IR、Argus tag、**TIRx**（TVM FFI）、Triton、StableHLO，或廠商 Tile。多底層機群等不起一份 mega-schema——但每條產品線需要*一個*選定的 **面向 LLM 的 IR**，不是把 IR 貼進去。
