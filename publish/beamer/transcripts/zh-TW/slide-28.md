...
**T1 — 型別化介面 → C3、C5、C6。**
Exists：CompileIQ 智慧體技能、**Cake IR**（型別化排程，沒有版面代數）、**Argus** tag 函式／斷言、**TIRx**（TVM FFI + tile primitive；B 級）、**IntOpt** 意圖序列（經典實現）、**LLM4IR** 作為*負例*（裸 LLVM 傾印在 CFG／執行上失敗）。主張：**面向 LLM 的 IR** 是智慧體可見的臉——schema，不是貼上然後祈禱。Missing：跨 MLIR、Triton、Tile、Cake、Argus、TIRx 的可攜摘要與行動——今天每個堆疊都重黏。解鎖窄 ACF／提示壓過自由改寫（C3），以及發行說明裡的具名預設路徑（C5）。
