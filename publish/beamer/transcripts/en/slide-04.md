...
KernelBench (Stanford / Princeton Scaling Intelligence) asks for correct *and* faster kernels; one-shot success is often under 20%, fusion remains hard. **GEAK v4** (AMD) is no longer a single-kernel loop: `e2e_workflow` Amdahl-triages a warm sglang/vLLM server, then recurses into kernel search only for kernels that move throughput, with warm-server **A/B** and output parity. **Cake** (NVIDIA / CMU) has agents author a typed **Cake IR** schedule — warp roles, barriers, memory tiers — and beats matched CUDA/PTX search; Kimi Delta Attention is SGLang-validated. **Argus** (CausalFlow / HKUST / Stanford et al.) uses tag functions and compile-time **SMT** (satisfiability modulo theories) over a layout algebra; authors report 99–104% of MI300X hand-optimized assembly **TFLOPS** on GEMM / attention / MoE. Closing beat: refinement still raises correctness more reliably than speed on public ladders — peak library-class wins stay family-selected (**C2** open).

**E — Verification enters the loop.**
Stack the oracles: unit and golden tests, numerical checks versus reference, Alive2-class local formal (UIUC lineage; LLM-VeriOpt-style rewards), **Argus** compile-time SMT plus thread-level counterexamples, and **T-LLM Compiler** (Huawei) chaining Alive2 + **CBMC** (C Bounded Model Checker) on PolyBench/C. Strong locally; weak on **GPU** races and floating-point nondeterminism. Admit for money needs this ladder, not a single check.
