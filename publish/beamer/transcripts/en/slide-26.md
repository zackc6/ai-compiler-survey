...
Workflow compile, ADG check, freeze, place — T10, gap 4.6. FlowCompile, Auto, AgentFlow compile or analyze the graph; **SIGIL** ★ compiles skills to a typed harness; **SKILL.state** is P2 (\(\Sigma_t\)); **DeepSeek Harness** is a live runtime (plugins + session-log replay). Shared *compiler-product* agent-graph IR with fail-closed CI is still Horizon B. AG-IR / SkIR are skill IRs, not T1.
