...
4.6 workflow / skill compile: ADG (Agent Dependency Graph — static IR of the agent program) check, freeze, place — T10 path to Horizon B. FlowCompile and AgentFlow *compile or analyze* the graph; **SIGIL** compiles `SKILL.md` → AG-IR → a harness; **DeepSeek Harness** *runs* a live plugin tree with a session log; **SKILL.state** keeps explicit \(\Sigma_t\) instead of a growing transcript. Skill IRs are not T1. Fail-closed **compiler** product CI is still missing. 4.8 human-review process: CODEOWNERS, signed admit, HITL (human-in-the-loop) capacity — agents multiply drafts faster than review scales. 4.9 security: sandbox, provenance, supply chain — agent kernels in the TCB need the same bar as hand-written code. SkCC Anti-Skill Injection is T9 color for untrusted `SKILL.md`, not kernel CODEOWNERS.
