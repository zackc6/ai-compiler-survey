...
**3. Agent-visible IR: dump vs summaries / Cake / Argus / TIRx / Tile?**
Where will you invest T1 typed interfaces first — and *what face* will the LLM see? Dumping LLVM/MLIR (LLM4IR: CFG/exec fail) vs summaries, IntOpt-style intent, Cake IR, Argus tags, **TIRx** (TVM FFI), Triton, StableHLO, or vendor Tile. Multi-substrate fleets cannot wait for one mega-schema — but you need *a* chosen **LLM-oriented IR** per product line, not paste-the-IR.
