...
Exists: CompileIQ agent skills, **Cake IR** (typed schedule, no layout algebra), **Argus** tag functions / assertions, **Zomboss** mapping surface / intent / resolved plan. Claim: agents need schemas, not paste-and-pray. Missing: portable summaries and actions across MLIR, Triton, Tile, Cake, Argus — today every stack re-glues. Unlocks narrow ACFs/hints over free rewrite (C3) and named default paths in release notes (C5).
