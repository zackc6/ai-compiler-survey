...
Exists: CompileIQ agent skills, **Cake IR** (typed schedule, no layout algebra), **Argus** tag functions / assertions, **IntOpt** intent sequences (classical realize), **LLM4IR** as the *negative* (raw LLVM dump fails CFG/exec). Claim: **LLM-oriented IR** is the agent-visible face — schemas, not paste-and-pray. Missing: portable summaries and actions across MLIR, Triton, Tile, Cake, Argus — today every stack re-glues. Unlocks narrow ACFs/hints over free rewrite (C3) and named default paths in release notes (C5).
